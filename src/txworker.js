...
self.importScripts('http://localhost:1024/jsenscript.min.js')
self.importScripts('http://localhost:1024/crypto.js')
...
self.saveToBackward = async function(result) {
    ...
    return new Promise((resolve, reject) => {
        self.isSaving = true;
        fetch(`${self.location.origin}/rrweb/saveEvent`, {
            method: 'POST', // *GET, POST, PUT, DELETE, etc.
            headers:{
                'Content-Type': 'application/json'
            },
        ...
        .then(async (resp) => {
            this.console.log('hasSavedData:', resp.status, resp.body);
            if(resp.status === 200) {
                const result = await self.removeSavedDataFromeIndexsDB();
                resolve(result);
            }else {
                reject(result);
            };
